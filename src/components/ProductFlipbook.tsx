  ...
  useEffect(() => {
    ...
    async function initialise() {
      try {
        ...
        const catalogueTitle = catalogue?.title || 'Product Catalogue 2026';
        ...
        const pages = Array.from({ length: pdf.numPages }, (_, index) => {
          ...
          const img = document.createElement('img'); img.alt = `Catalogue page ${index + 1}`;
        ...
        const renderPage = (index: number): Promise<void> => {
          ...
          const task = pdf.getPage(index + 1).then(async (pdfPage) => {
            ...
            const blob = await new Promise<Blob>((resolve, reject) => canvas.toBlob((value) => value ? resolve(value) : reject(new Error('Page render failed.')), 'image/jpeg', 0.9));
            if (cancelled) return;
        ...
        setStatus('Swipe the book or use the controls to browse the catalogue.');
      } catch (error) { console.error('Unable to load the product catalogue flipbook.', error); setStatus('The catalogue could not be loaded. Please refresh the page.'); }
    }
    void initialise();

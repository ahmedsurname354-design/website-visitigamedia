...
export default function NewsComments({ newsId, admin = false }: { newsId: string; admin?: boolean }) {
  ...
  const load = useCallback(async (cursor?: NewsComment) => {
    ...
    try {
      ...
      if (active.current && request === generation.current) setLoadError('Komentar belum dapat dimuat. Silakan coba lagi.');
    } finally {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if ([...name.trim()].length < 2 || [...name.trim()].length > 80 || !body.trim() || [...body.trim()].length > 2000) {
      setError('Isi nama 2–80 karakter dan komentar 1–2.000 karakter.');
      return;
    }
    ...
    try {
      ...
      setBody(''); setSuccess('Komentar berhasil dikirim.');
      await load();
    } catch (reason) {
      if (active.current) setError(reason instanceof Error ? reason.message : 'Komentar belum terkirim. Silakan coba lagi.');
    } finally {
  ...
  return <section aria-label="Komentar artikel" className="text-[#241811]">
    {!admin && <><h2 className="text-2xl font-bold">Komentar</h2><p className="mt-2 text-sm text-[#735c4d]">Bagikan pendapat Anda dengan mengisi nama dan komentar.</p>
      <form onSubmit={(event) => void submit(event)} className="mt-5 space-y-4">
        <label className="block text-sm font-semibold">Nama<input name="name" required minLength={2} maxLength={80} autoComplete="name" value={name} disabled={busy} onChange={(event) => setName(event.target.value)} className={fieldClass} /></label>
        <label className="block text-sm font-semibold">Komentar<textarea name="body" required maxLength={2000} rows={4} value={body} disabled={busy} onChange={(event) => setBody(event.target.value)} className={fieldClass} /></label>
        ...
        <button type="submit" disabled={busy || loading} className="editorial-button editorial-button--primary disabled:opacity-50">{busy ? 'Mengirim…' : 'Kirim komentar'}</button>
      </form></>}
    ...
    <div className="mt-6 space-y-4" aria-busy={loading}>
      ...
      {loading && <p role="status" className="text-sm text-[#735c4d]">Memuat komentar…</p>}
      {!loading && !loadError && items.length === 0 && <p className="text-sm text-[#735c4d]">Belum ada komentar.</p>}
      {loadError && <div role="alert"><p className="text-sm text-red-700">{loadError}</p><button type="button" disabled={loading || busy} onClick={() => void load()} className="mt-2 rounded-lg border px-4 py-2 text-sm font-semibold">Coba lagi</button></div>}
      {hasMore && !loadError && <button type="button" disabled={loading || busy} onClick={() => void load(items[items.length - 1])} className="rounded-xl border border-[#ddc9b8] px-5 py-3 text-sm font-semibold disabled:opacity-50">Muat lainnya</button>}
    </div>
  </section>;
}

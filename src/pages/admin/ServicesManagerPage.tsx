...
export default function ServicesManagerPage() {
  ...
  const submit = async (form: FormData) => {
    ...
    try {
      ...
      await saveServiceContent(next); setContent(next); setMessage('Konten layanan berhasil disimpan.');
  ...
  return <div className="mx-auto max-w-7xl"><div className="mb-8"><p className="text-xs font-bold uppercase tracking-[.2em] text-orange-600">Konten website</p><h2 className="mt-2 text-3xl font-black text-slate-900">Layanan</h2><p className="mt-2 text-slate-500">Atur judul, kartu layanan, tombol, poster, dan video showreel.</p></div>
    ...
    <form onSubmit={(event) => { event.preventDefault(); void submit(new FormData(event.currentTarget)); }} className="space-y-7">
      ...
      <Section title="Media showreel"><div className="grid gap-5 lg:grid-cols-2"><VideoUpload label="Video WebM" accept="video/webm,.webm" value={content.video_webm_url} onChange={(url) => setContent((current) => ({ ...current, video_webm_url: url }))} /><VideoUpload label="Video MP4" accept="video/mp4,.mp4" value={content.video_mp4_url} onChange={(url) => setContent((current) => ({ ...current, video_mp4_url: url }))} /><Field name="video_poster_url" label="URL poster video" initial={content.video_poster_url} className="lg:col-span-2" /></div>{content.video_mp4_url && <video controls preload="metadata" poster={content.video_poster_url} className="mt-5 aspect-video w-full max-w-2xl rounded-xl bg-black"><source src={content.video_webm_url} type="video/webm" /><source src={content.video_mp4_url} type="video/mp4" /></video>}</Section>
...
function VideoUpload({ label, accept, value, onChange }: { label: string; accept: string; value: string; onChange: (url: string) => void }) {
  const [uploading, setUploading] = useState(false); const [error, setError] = useState('');
  const upload = async (file?: File) => { if (!file) return; if (file.size > 30 * 1024 * 1024) { setError('Video maksimal 30 MB.'); return; } if (!supabase) { setError('Supabase belum dikonfigurasi.'); return; } setUploading(true); setError(''); try { const extension = file.name.split('.').pop()?.toLowerCase() || 'mp4'; const path = `services/${crypto.randomUUID()}.${extension}`; const { error: uploadError } = await supabase.storage.from('website-media').upload(path, file, { cacheControl: '31536000', contentType: file.type, upsert: false }); if (uploadError) throw uploadError; onChange(supabase.storage.from('website-media').getPublicUrl(path).data.publicUrl); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Video gagal diunggah.'); } finally { setUploading(false); } };
  return <div><p className="text-sm font-medium text-slate-700">{label}</p><p className="mt-2 truncate rounded-xl bg-slate-50 px-3 py-2 text-xs text-slate-500">{value}</p><label className="mt-3 inline-flex cursor-pointer items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold hover:bg-slate-50"><input type="file" accept={accept} className="sr-only" disabled={uploading} onChange={(event) => void upload(event.target.files?.[0])} />{uploading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}{uploading ? 'Mengunggah…' : 'Pilih video'}</label>{error && <p className="mt-2 text-xs text-red-600">{error}</p>}</div>;
}

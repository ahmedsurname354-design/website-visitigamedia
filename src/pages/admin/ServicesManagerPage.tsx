...
export default function ServicesManagerPage() {
  ...
  return <div className="mx-auto max-w-7xl"><div className="mb-8"><p className="text-xs font-bold uppercase tracking-[.2em] text-orange-600">Konten website</p><h2 className="mt-2 text-3xl font-black text-slate-900">Layanan</h2><p className="mt-2 text-slate-500">Atur judul, kartu layanan, tombol, poster, dan video showreel.</p></div>
    ...
    <form onSubmit={(event) => { event.preventDefault(); void submit(new FormData(event.currentTarget)); }} className="space-y-7">
      ...
      <Section title="Media showreel"><div className="grid gap-5 lg:grid-cols-2"><VideoUpload value={content.video_mp4_url} onChange={(url) => setContent((current) => ({ ...current, video_mp4_url: url }))} /><Field name="video_poster_url" label="URL poster video" initial={content.video_poster_url} /></div>{content.video_mp4_url && <video controls preload="metadata" poster={content.video_poster_url} className="mt-5 aspect-video w-full max-w-2xl rounded-xl bg-black"><source src={content.video_mp4_url} type="video/mp4" /></video>}</Section>
